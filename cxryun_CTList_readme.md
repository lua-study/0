...
```
# 直接运行
/path/to/CTList -a " " -bind 0.0.0.0 -port 80

# 不打印HTTP访问日志
/path/to/CTList -a " " -bind 0.0.0.0 -port 80 -l

# 后台运行 (Windows操作系统无效)
/path/to/CTList -a " " -bind 0.0.0.0 -port 80 -d

# 以下开机自启方式二选一, 推荐 SYSTEMD 方式.
# 开机自启 (CRON)
编辑 /etc/crontab 文件, 并在文件末尾多添加几个空行. (有些系统不留空行会出现不能自启动)
@reboot root /path/to/CTList -a " " -bind 0.0.0.0 -port 80 -d

# 开机自启 (SYSTEMD)
cat > /etc/systemd/system/ctlist.service  " -bind 0.0.0.0 -port 80 -l
Restart=on-failure
...
Token(登陆保活): 60 * 60 * 10
Cookie(会话授权): 60 * 30
RefreshURL(真实下载链接): 189 （配置文件可改  ）
RefreshInterval(刷新目录结构): 60 * 15  （配置文件可改, 全局最小值生效  ）
```
