...
```xml
             ...
             com.isea533.mybatis.mapper.Mapper 
...
```
一旦继承了`Mapper `,继承的`Mapper`就拥有了以下通用的方法:
```java
...
如果想在Spring4中使用泛型注入，还需要包含`Mapper `所在的包，具体请看 [在Spring4中使用通用Mapper](http://git.oschina.net/free/Mapper/blob/master/UseMapperInSpring4.md)。  
