...
Here, special thanks to Jeff from Github and a friend naming "Canton - 201103L" from CrossApp official QQ group for their submitted optimizations. 

#####7. Fixed bug that system act touchBegin two times when touch event dispatch sometimes 
Here, special thanks to a friend naming “Juejinshierzuofei” from CrossApp official QQ group for his submitted optimization. 
...
Text message model class, supports to obtain corresponding height, width and other info after CALabel text rendering. 
...
Optimized DownloadManager, fixed bug that user cannot instant stop and delete download order 
...
Optimized file download management system and fixed header file reference bug from previous release 

#####5. Rearranged device related system interface
For example: GPS info obtaining, camera and photo album calling and address book obtaining etc. 
...
####Suggestion:
rebuild project and copy files in Classes and Resources 
...
Basic support for all functionalities of CrossApp 0. 3. 8 

#####2. Added CATextView
multi-line text input control, its usage is similar with CATextField 

#####3. Added DownloadManager
file download management system 
...
Added pull up and pull down refresh plug-ins of CAListView, CATableView and CACollectionView, please view their usage in CollectionViewTest of demo. Removed pull up and pull down refresh monitoring agent of older version CATableView. 

##### 2. Added page turning effect of left and right swipe gestures
You are required to manually set relative properties to launch this effect. 

##### 3. Touch Event Distributed System Optimization
Added functionality that supports system auto switching to monitor parent container after child container slides to end when same direction sliding container is nested. 
...
#### Suggestion:
Note seventh item’s amendment, if you use this agent method, you should follow the same name of it. 
...
##### 1. Added CAStepper
A number input control which is able to progressively input number. 

##### 2. Modified CAAlertView Interface
Rewrote the interface. 

##### 3. CATextField Optimization
Fixed bug and added agent monitoring method. 
...
##### 2. Added CAListView
CATableView’s simplified version, and added horizontal arrangement functionality 

##### 3. Added CAPageControl
pageView page progress indicator 

##### 4. CAActivityIndicatorView Optimization
Added customized image interface, and added customized background image interface 

##### 5. CAImageView Optimization
Added image adaptation mode interface which supports: fully-stretched, fully-stretched with no distortion and cut excess part, fully-displayed with no distortion and centered, fully-displayed with no distortion and stay same height, fully-displayed with no distortion and stay same width. 

##### 6. CATextField Optimization
Modified background setting interface 

##### 7. CASegmentedControl Optimization
Added interfaces and fixed some bugs 
...
Added cache mechanism, text performance had another improvement. 

#####3. CATabBar Optimization
Added some methods and interfaces. 

#####4. CATableView and CACollectionView Optimization
Added section’s Header and Footer’s hovering effect. 

#####5. CATextField Optimization
Optimized and fixed some bugs. 

#####6. Other CAVector, CADeque, CAList
Optimized some interfaces. Added the mutual transformation among three containers. 
...
Rewrite-version textField has a better efficiency 

#####4. CAPageView Optimization
Added callmonitoring current selected page of callback method for CAPageView 

#####5. CALabel Optimization
Added sizetoFit method for modifying label’s own size into text actual size 
...
1.Added keyboard type selecting on text field 
2.Added hiding display character ※ 
3.Added cursor movement, insert and delete 
4.Added functions such as obtaining keyboard height and others 
5.Only support iOS and Android for now 

#####5. Added new functions of CATabBarController and CANavigationController
Support tabBarController switching to viewController animation effect 
Added TabBar and NavigationBar hiding function 

#####6. Fixed some bugs
1.Fixed bug: setting ZOder before addSubview in CAView is invalid 
2.Fixed bug: calling setNotificationNode(CAView* view) in CCDirector is invalid 
3.Fixed bug: scrollView indicator's display and hiding timing are wrong 
...
The new touch event does not have to be registered, and you can implement cctouchBegin and other functions monitoring by only inheriting all child classes of CAView. In addition, it’s unable to obtain event monitoring outside of view rectangular region when you touch screen. 

The new touch event is dispatched from parent view to child view, so even if your touchBegin event spot is in child view rectangular region, you cannot obtain touch event on condition that the touchBegin event is outside of parent view rectangular region. 
...
**a)	Simplify usage: ** start, stop and other operation can be acted by calling corresponding static methods. 
**b) Memory management modification: ** previously when we start timer, reference counting on pTarget will plus 1 for the purpose of avoiding program crashed caused by pTarget release. Now the new mechanism we deployed could completely solve the above problem: this mechanism enables system to stop plus 1 on pTarget reference counting after timer starts and auto-stop related timer after pTarget release. 
**c) New feature: ** when pTarget is derived type of CAViewController or others and the view of CAViewController is not displayed, timer will auto pause; it will auto recover when the view is displayed. In addition, it will not change before pTarget releases. 
...
Reminder: CAAlertView 
View paging controller: CAPageControl 
Blocks of text input control: CATextView 
Load status controller: CAActivity 
Webpage load control: CAWebView 
...
#####1.	textField：
**a) ** free control on text content length 
**b) ** text editing when cursor is at anywhere of content 
**c) **line feed input 
#####2.	tableView：
**a) **cell reuse 
**b) **cell editing (delete, insert and others) 
**c) **added some template attributes of cell 
