...
5.  *This step is optional.*
    ...
    the `ycm_core` library:
        ...
        mkdir regex_build
        cd regex_build
        cmake -G " " . ~/.vim/bundle/YouCompleteMe/third_party/ycmd/third_party/cregex
        cmake --build . --target _regex --config Release

    where ` ` is the same generator used in the previous step.
...
**NOTE**: It is highly recommended to include `-x  ` flag to libclang.
...
application by typing `:ProjectCreate   -n ruby` inside vim
...
` ` key (or any other key defined by the `wildchar` option) to complete the
...
useful when the menu is blocking the view, when you need to insert the ` `
...
sure you don't have something mapped to ` `, ` ` or ` ` (in insert
mode).
...
Vim prevents remapping of the ` ` key in all ` ` completion modes
(except the `  `/`  ` mode which operates in the same mode as
` `/` `) and YCM uses the `  ` (`completefunc`) mode for
completions. This means that adding ` ` to any of the `g:ycm_key_list_*`
