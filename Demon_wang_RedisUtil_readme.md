...
|  11  | void multiSet(Map  maps)                            | 批量添加                              |
|  12  | boolean multiSetIfAbsent(Map  maps)                 | 批量添加，仅当所有key都不存在          |
|      |      **其他方法**                                                 |                                      |
...
|   2  | Set > zRangeWithScores(String key, long start, long end) | 获取集合元素, 并且把score值也获取  |
|   3  | Set  zRangeByScore(String key, double min, double max)              | 根据score范围查询元素,从小到大排序 |
|   4  | Set > zRangeByScoreWithScores(key,double min,double max) | 根据score范围查询元素,并返回score |
...
|   7  | Set > zReverseRangeWithScores(key, long start, long end) | 获取元素,从大到小排序,并返回score  |
...
|      |                                                                            |                                  |
|  21  | Cursor > zScan(String key, ScanOptions options)          | 使用迭代器获取                    |
|      |    **添加相关**                                                            |                                 |
|  22  | Boolean zAdd(String key, String value, double score)                       | 添加元素,zSet按score由小到大排列  |
|  23  | Long zAdd(String key, Set > values)                      | 批量添加,TypedTuple使用见下面介绍 |
|      |    **删除相关**                                                           |                                  |
