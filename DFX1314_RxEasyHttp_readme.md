...
链式调用的终点请求的执行方式有：execute(Class  clazz) 、execute(Type type)、execute(CallBack  callBack)三种方式，都是针对标准的ApiResult
...
EasyHttp.get(url)
                ...
                //.execute(new TypeToken >() {}.getType())//Type类型
...
*注：目前通过execute(Class  clazz)方式只支持标注的ApiResult结构，不支持自定义的ApiResult*
示例：
```
...
### 请求回调CallBack支持的类型
```
//支持回调的类型可以是Bean、String、CacheResult 、CacheResult 、List 
new SimpleCallBack >()//支持缓存的回调，请看缓存讲解
new SimpleCallBack >()//支持缓存的回调，请看缓存讲解
...
```
### 缓存回调
对具有缓存的回调CallBack，如果你想知道当前的缓存是来自本地还是网络，只需要回调中加入CacheResult，其它和普通的网络请求方式一模一样。CacheResult中的isFromCache可以知道是否来自缓存，true：来自缓存，false：来自网络。请使用`new SimpleCallBack >()` 也就是在你原有的T上包含一层CacheResult就可以了。如果不想用到isFromCache就不需要用CacheResult，直接使用`new SimpleCallBack ()`
带有CacheResult回调示例：
```
 EasyHttp.get(url)
...
4.在----response----之后会打印（在服务器响应后被打印），包含响应码、响应状态、响应头、cookie,body等以  true保存缓存的状态
## 混淆
```
#okhttp
