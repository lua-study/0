...
int maxId1 = db.Queryable ().Max(it => it.id).ObjToInt();
int maxId2 = db.Queryable ().Max ("id"); 

//get min id
int minId1 = db.Queryable ().Where(c => c.id > 0).Min(it => it.id).ObjToInt();
int minId2 = db.Queryable ().Where(c => c.id > 0).Min ("id");


//order By 
var orderList = db.Queryable ().OrderBy("id desc,name asc").ToList();
//order by 
...
var list6 = db.Queryable ().In(intList).ToList(); //in primary fileds

//group by
var list7 = db.Queryable ().Where(c => c.id   it.sex).Select("sex,count(*) Count").ToDynamic();
var list8 = db.Queryable ().Where(c => c.id   it.sex).GroupBy(it => it.id).Select("id,sex,count(*) Count").ToDynamic();
List  list9 = db.Queryable ().Where(c => c.id   it.sex).Select ("Sex,count(*) Count").ToList();
List  list10 = db.Queryable ().Where(c => c.id  ("Sex,count(*) Count").ToList();
//SELECT Sex,Count=count(*)  FROM Student  WHERE 1=1  AND  (id  ()
...
List  jList4 =
	db.Queryable ()
	.JoinTable ((s1, s2) => s1.sch_id == s2.id) // left join  School s2  on s1.id=s2.id
	.JoinTable ((s1, s2, a1) => a1.id == s2.AreaId)// left join  Area a1  on a1.id=s2.AreaId  
		.JoinTable ((s1, a1, s3) => a1.id == s3.AreaId)// left join  School s3  on a1.id=s3.AreaId  
...
//Join child
var childQuery = db.Queryable ().Where("id=@id").Select(it => new { id = it.id }).ToSql();//create child SQL
...
var r1 = db.Queryable ().Where(it => it.name == par1.ObjToString()).ToList(); //ObjToString if null return ""
var r2 = db.Queryable ().Where(it => it.d1 == par1.ObjToDate()).ToList();
var r3 = db.Queryable ().Where(it => it.id == 1.ObjToInt()).ToList();//ObjToInt if null return 0
var r4 = db.Queryable ().Where(it => it.id == 2.ObjToDecimal()).ToList();
var r5 = db.Queryable ().Where(it => it.id == 3.ObjToMoney()).ToList();
var r6 = db.Queryable ().Where(it => it.v1 == par2.Trim()).ToList();
...
var convert3 = db.Queryable ().Where(c => c.name == par2.ToLower()).ToList();
var convert4 = db.Queryable ().Where(c => c.name == par2.ToUpper()).ToList();
var convert5= db.Queryable ().Where(c => DateTime.Now > Convert.ToDateTime("2015-1-1")).ToList();
var c1 = db.Queryable ().Where(c => c.name.Contains("a")).ToList();
var c2 = db.Queryable ().Where(c => c.name.StartsWith("a")).ToList();
var c3 = db.Queryable ().Where(c => c.name.EndsWith("a")).ToList();
var c4 = db.Queryable ().Where(c => !string.IsNullOrEmpty(c.name)).ToList();
var c5 = db.Queryable ().Where(c => c.name.Equals("小杰")).ToList();
var c6 = db.Queryable ().Where(c => c.name.Length > 4).ToList();
var time = db.Queryable ().Where(c => c.d1>DateTime.Now.AddDays(1)).ToList();
var time2 = db.Queryable ().Where(c => c.d1 > DateTime.Now.AddYears(1)).ToList();
var time3 = db.Queryable ().Where(c => c.d1 > DateTime.Now.AddMonths(1)).ToList();
var intList = intArray.ToList();
var list0 = db.Queryable ().In(it => it.id, 1,2,3).ToList();
var list1 = db.Queryable ().In(it=>it.id, intArray).ToList();
var list2 = db.Queryable ().In("id", intArray).ToList();
...
var list7 = db.Queryable ().Where(c => c.id   it.sex).Select("sex,Count=count(*)").ToDynamic();
var list8 = db.Queryable ().Where(c => c.id   it.sex).GroupBy(it=>it.id).Select("id,sex,Count=count(*)").ToDynamic();
List  list5 = db.Queryable ().Where(c => c.id   it.sex).Select ("Sex,Count=count(*)").ToList();
List  list6 = db.Queryable ().Where(c => c.id      ("Sex,Count=count(*)").ToList();
//SELECT Sex,Count=count(*)  FROM Student  WHERE 1=1  AND  (id  ()
	.JoinTable ((s1, s2) => s1.sch_id == s2.id) //detault left join
		.Where ((s1, s2) => s1.id == 1)
			.Select("s1.*,s2.name as schName")
			.ToDynamic();
...
//join by page
var jList2 = db.Queryable ()
	.JoinTable ((s1, s2) => s1.sch_id == s2.id) //default left join
		//inner join
		//.JoinTable ((s1, s2) => s1.sch_id == s2.id  ,JoinType.INNER)
		.Where ((s1, s2) => s1.id > 1)
			.OrderBy ((s1, s2) => s1.name)
				.Skip(10)
				.Take(20)
				.Select("s1.*,s2.name as schName")
				.ToDynamic();
...
var jList3 = db.Queryable ()
	.JoinTable ((s1, s2) => s1.sch_id == s2.id) // left join  School s2  on s1.id=s2.id
		.Where ((s1, s2) => s1.id > 1)  // where s1.id>1
			.OrderBy ((s1, s2) => s1.id) //order by s1.id no one  ordder   .oderBy().orderby  
				.Skip(1)
				.Take(2)
				.Select ((s1, s2) => new classNew() { newid = s1.id, newname = s2.name, xx_name = s1.name }) 
					.ToList();
```
...
//get int
var list5 = db.SqlQuery ("select top 1 id from Student").SingleOrDefault();
//get dictionary
Dictionary  list6 = db.SqlQuery >("select id,name from Student").ToDictionary(it => it.Key, it => it.Value);
//get List 
var list7 = db.SqlQuery ("select top 1 id,name from Student").SingleOrDefault();
//get sp result
...
var spResult2 = db.SqlQuery ("exec sp_school @p1,@p2 output", pars);
...
var spResult3 = db.SqlQuery ("sp_school", pars2);
...
List  dataList = db.Sqlable()
	...
	.Where("s.id>100 and s.id ("st.*", new { id = 1 });
...
List  dataPageList = db.Sqlable()
	...
	.Join("student", "st", "st.id", "s.id", JoinType.INNER)
	.Join("student", "st2", "st2.id", "st.id", JoinType.LEFT)
	.Where("s.id>100 and s.id ("st.*", "s.id", 1, 10);
...
db.Update (new { name = "蓝翔14" }, it => it.id == 14); //only update name
db.Update (new { name = "蓝翔11 23 12", areaId = 2 }, 11, 23, 12);
db.Update (new { name = "蓝翔2" }, new string[] { "11", "21" });
db.Update (new { name = "蓝翔2" }, it => it.id == 100);
var array=new int[]{1,2,3};
db.Update (new { name = "蓝翔2" }, it => array.Contains(it.id));// id in 1,2,3
...
db.Update (new School { id = 12, name = "蓝翔12", AreaId = 2 }, it => it.id == 18);
...
//delete by exp
db.Delete (it => it.id > 100);//support it=>array.contains(it.id)

//delete by primary key values
db.Delete (new string[] { "100", "101", "102" });

//delete by field values
db.Delete (it => it.name, new string[] { "" });
db.Delete (it => it.id, new int[] { 20, 22 });
...
public class MappingTable : IDemos
{
	...
	public void Init()
	{
		...
		{
			var list = db.Queryable ().ToList();//查询的是 select * from student 而我的实体名称为V_Student
		}
	}
	...
	{
		//key类名 value表名
		public static List  MpList = new List (){
...
public class MappingColumns : IDemos
{
	...
	public void Init()
	{
		...
		using (var db = SugarFactory.GetInstance())
		{
			var list = db.Queryable ().Where(it=>it.classId==1).ToList();
		}
	}
