...
The whitepaper for SVT-HEVC can be found here:  
...
- __Build Instructions__
  - Build the project by following the steps below in a windows command prompt:
    - In the main repository directory, cd to ` \Build\windows`
...
- __Build Instructions__
  ...
  - or run the commands directly
    ...
    ``` bash
    mkdir build && cd build && cmake .. -DCMAKE_BUILD_TYPE=  && make -j $(nproc) && sudo make install
    ```
