...
Once we got the project location, the macro `$(VIM_ROOT)`, or its alias ` `, can be used to represent the project root:
...
[project-run]
command=make run
#   is an alias to `$(VIM_ROOT)`, a little easier to type.
cwd= 
output=terminal
```
...
```VimL
noremap    :AsyncTask project-run 
noremap    :AsyncTask project-build 
```
...
```ini
[grep]
command=rg -n --no-heading --color never "$(?keyword)" " " -tcpp -tc -tpy -tvim -tgo -tasm
cwd=$(VIM_ROOT)
errorformat=%f:%l:%m
```

Here is my global `grep` task. Each time I use `:AsyncTask grep` in any of my project, it prompts me to input `keyword` before searching, I can use `  ` to pickup word under cursor or input something new.
...
# no need to go back to project root directory, because "cwd= ".
