...
The configure method is very simple, just edit it by referring to current values (`boards/ /Makefile`), this command open local configuration (`boards/ /.labconfig`) via vim:
...
Apply available patches in `boards/ /bsp/patch/linux` and `patch/linux/`:
...
Configure one by one, by default, use the defconfig in `boards/ /bsp/`:
...
Save configs and images to `boards/ /bsp/`:
...
And more ` -list` are also supported with `list  `, for example:
...
List available modules in `modules/`, `boards/ /bsp/modules/`:

    $ make module-list

If `m` argument specified, list available modules in `modules/`, `boards/ /bsp/modules/` and `linux-stable/`:
...
Host:
    ...
    ROOTDIR="/labs/linux-lab/boards/ /bsp/root/ /rootfs"
...
Linux Lab allows to access Makefile goals easily via ` -run`, for example:
...
Please refer to `doc/qemu/qemu-doc.html` or the online one  .
...
The configuration must be put in `boards/ /` and named with necessary
...
variables should be configured in `boards/ /Makefile`.
...
If want v5.1 kernel, just put a line "LINUX = v5.1" in `boards/ /Makefile`.
