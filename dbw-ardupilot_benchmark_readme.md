...
BENCHMARK(BM_memcpy)->Arg(8)->Arg(64)->Arg(512)->Arg(1 Arg(8 Range(8, 8 RangeMultiplier(2)->Range(8, 8  data;
  for (auto _ : state) {
    state.PauseTiming();
    data = ConstructRandomSet(state.range(0));
    state.ResumeTiming();
    for (int j = 0; j  Args({1 Args({2 Args({4 Args({8 Args({1 Args({2 Args({4 Args({8 Ranges({{1 Args({i, j});
}
...
BENCHMARK(BM_StringCompare)
    ->RangeMultiplier(2)->Range(1 Complexity(benchmark::oN);
```
...
```c++
BENCHMARK(BM_StringCompare)
    ->RangeMultiplier(2)->Range(1 Complexity();
```
...
BENCHMARK(BM_StringCompare)->RangeMultiplier(2)
    ->Range(1 Complexity([](int64_t n)->double{return n; });
```
...
BENCHMARK_TEMPLATE(BM_Sequential, WaitQueue )->Range(1 
...
```c++
BENCHMARK(BM_test)->Range(8, 8 UseRealTime();
```
...
#pragma omp parallel for
  for(int i = 0; i  Range(8, 8 Range(8, 8 UseRealTime();

// Measure the total CPU consumption, use it to decide for how long to
// run the benchmark loop. This will always measure to no less than the
// time spent by the main thread in single-threaded case.
BENCHMARK(BM_OpenMP)->Range(8, 8 MeasureProcessCPUTime();

// A mixture of the last two. Measure the total CPU consumption, but use the
// wall clock to decide for how long to run the benchmark loop.
BENCHMARK(BM_OpenMP)->Range(8, 8 MeasureProcessCPUTime()->UseRealTime();
```
...
BENCHMARK(BM_ManualTiming)->Range(1, 1 UseManualTime();
```
...
static void BM_test(benchmark::State& state) {
  for (auto _ : state) {
      int x = 0;
      for (int i=0; i  )` forces the  *result* of ` ` to be stored in either
...
void BM_spin_empty(benchmark::State& state) {
  for (auto _ : state) {
    for (int x = 0; x  ComputeStatistics("max", [](const std::vector & v) -> double {
