...
Dictionary  diskFree = SystemInfo.DiskFree();// 获取磁盘每个分区可用空间
Dictionary  diskTotalSpace = SystemInfo.DiskTotalSpace();// 获取磁盘每个分区总大小
Dictionary  diskUsage = SystemInfo.DiskUsage();// 获取磁盘每个分区使用率
double temperature = SystemInfo.GetCPUTemperature();// 获取CPU温度
int cpuCount = SystemInfo.GetCpuCount();// 获取CPU核心数
IList  ipAddress = SystemInfo.GetIPAddress();// 获取本机所有IP地址
string localUsedIp = SystemInfo.GetLocalUsedIP();// 获取本机当前正在使用的IP地址
IList  macAddress = SystemInfo.GetMacAddress();// 获取本机所有网卡mac地址
...
double time = HiPerfTimer.Execute(() =>
{
    for (int i = 0; i  > where1 = s => s.StartsWith("a");
Expression > where2 = s => s.Length > 10;
Func  func = where1.And(where2).Compile();
bool b=func("abcd12345678");//true
```
```csharp
Expression > where1 = s => s.StartsWith("a");
Expression > where2 = s => s.Length > 10;
Func  func = where1.Or(where2).Compile();
...
.NET Framework
```csharp
MemoryStream ms = SevenZipCompressor.ZipStream(new List ()
{
    @"D:\1.txt",
...
```
```csharp
SevenZipCompressor.Zip(new List ()
{
    @"D:\1.txt",
    "http://ww3.sinaimg.cn/large/87c01ec7gy1fsq6rywto2j20je0d3td0.jpg",
}, zip);//压缩成zip
...
List  files = ftpClient.GetFiles("/");//列出ftp服务端文件列表
...
```
### 17.多线程后台下载
```csharp
...
### 21.HTML操作
```csharp
List  srcs = "html".MatchImgSrcs().ToList();// 获取html字符串里所有的img标签的src属性
...
Tuple > ipAddressInfo = "114.114.114.114".GetIPAddressInfo().Result;// 获取详细地理信息集合
```
### 24.元素去重
```csharp
var list = new List ()
...
List  classes = list.DistinctBy(c => c.Email).ToList();
...
```
```csharp
ExpressionMapper.CreateMap ();// 默认关系映射
ExpressionMapper.CreateMap ().ForMember(s => s.ClassC.PropertyName, d => d.CustomName, true);// 自定义关系映射
...
ExpressionMapper.CreateMap ().ConstructUsingServiceLocator().ReverseMap();// 链式自定义关系映射和反向映射
```
测试class：
```csharp
...
ExpressionMapper.CreateMap ().ReverseMap();
ExpressionMapper.CreateMap ().ReverseMap();
...
```
```csharp
Dictionary  dic1 = typeof(MyEnum).GetDictionary();// 获取枚举值和字符串表示的字典映射
...
### 27.定长队列实现
```csharp
LimitedQueue  queue = new LimitedQueue (32);// 声明一个容量为32个元素的定长队列
ConcurrentLimitedQueue  queue = new ConcurrentLimitedQueue (32);// 声明一个容量为32个元素的线程安全的定长队列
```
### 28.反射操作
```csharp
...
var data=new List >()
{
     new WeightedItem ("A", 1),
     new WeightedItem ("B", 3),
     new WeightedItem ("C", 4),
     new WeightedItem ("D", 4),
};
var item=data.WeightedItem();//按权重选出1个元素
var list=data.WeightedItems(2);//按权重选出2个元素
```
```csharp
var selector = new WeightedSelector (new List >()
{
    new WeightedItem ("A", 1),
    new WeightedItem ("B", 3),
    new WeightedItem ("C", 4),
