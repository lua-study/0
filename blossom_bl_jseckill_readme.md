...
如果你改成了"/", 或者其它的值。你需要登陆 http://localhost:15672 控制台去查看有效的virtual_host是多少。这里必须跟控制台的virtual_host
...
正确的配置是 spring.redis.password= 
而不是 spring.redis.password=''  。切忌在配置里加多余的单引号或者双引号。
