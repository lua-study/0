...
## 0.5 生产第一，自嗨顺便
只图写个一个玩具自嗨（练手）的话，应该什么技术都用最新、酷、炫，甚至最难的，比如能模板元就模板元……但要用于生产，除了简单，还要稳定、便捷。所以基本上只要有一台Ubuntu 18.04或更高版本的服务器（国内云服务器提供商都可提供），就可以简单几行指令准备好da4qi4的开发及部署环境。详见：[二、如何构建](#二如何构建)。
...
虽然说它“不干活”，但其实这个Web Server的运行表现合乎逻辑：我们没有为它指定任何操作绑定，所以对它的任何访问都返回“Not Found”（404）。
...
发生的变化：使用Aplication类的静态成员函数，定制（Customize）了一个应用，例中命名为app；然后改用它来添加前端以GET方法访问网站根URL路径时的处理方法。最后在svc运行之前，需要先将该应用挂接（Mount）上去。
...
```html
     ...
     你好，{=name=}！ 
     您正在使用的浏览器： {=_HEADER_("User-Agent")=} 
     您正在通过该网址访问本站：{=_HEADER_("Host")=} 
...
int main()
{
    ...
    {
        std::cerr  info("准备web应用中……"); //纯粹是为了演示日志……
