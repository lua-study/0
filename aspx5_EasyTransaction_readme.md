...
## 一、由来
这个框架是结合公司之前遇到的分布式事务场景以及 支付宝程立分享的一个PPT 而设计实现。
...
对于业务发起者，框架只暴露一个接口
	 ...
	 * @return
	 */
	 , E extends EasyTransExecutor, R extends Serializable> Future  execute(P params);
    }
...
如提供TCC服务的业务，需实现TccMethod接口：
     ...
     public interface TccMethod , R  extends Serializable> extends RpcBusinessProvider  {
    	R doTry(P param);
...
具体实现：
    ...
    @Component
    public class WalletPayTccMethod implements TccMethod {
