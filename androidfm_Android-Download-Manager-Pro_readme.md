...
public List  downloadTasksInSameState(int state);
public List  lastCompletedDownloads();
...
Example:
```java
List  report = dm.downloadTasksInSameState(TaskState.INIT);
```
...
Example:
```java
List  completedDownloadTasks = dm.lastCompletedTasks();
```
