...
- 作为播放器：
    ...
    weak_ptr  weakPlayer = player;
    player->setOnPlayResult([weakPlayer](const SockException &ex) {
        InfoL  getTrack(TrackVideo);
        if (!viedoTrack) {
            WarnL  addDelegate(std::make_shared ([](const Frame::Ptr &frame) {
            //此处解码并播放
        }));
    });

    player->setOnShutdown([](const SockException &ex) {
        ErrorL  play("rtsp://admin:jzan123456@192.168.0.122/");
	```
- 作为代理服务器：
	```cpp
...
- 作为推流客户端器：
	...
	NoticeCenter::Instance().addListener(nullptr,Config::Broadcast::kBroadcastRtmpSrcRegisted,
		...
		const_cast (pusher).reset(new RtmpPusher(app,stream));
